...
Rules:
- **Engine is N→M from day 1** (data model). First shipped *recipe* may be 1→1 (scope ramp). Don't confuse the two — one is permanent, one is temporary. When a recipe emits **multiple outputs** `a1…aN`, which of the two mechanisms applies (fixed M sibling columns via `output_slot` vs. a variable-length list-in-cell) is resolved in **§16 #9** — read it before adding any recipe with >1 output.
...
9. **A recipe on column A returns `a1, a2 … aN` — resolved: two distinct mechanisms, never conflated; the engine picks by *when N is known*, not by row data.** "A recipe returns multiple outputs" is ambiguous between two cases the engine already handles differently. Pin both, because conflating them is the trap:

   **(a) Fixed, design-time M → the N→M engine (`output_slot`).** The recipe's `output_schema` (§3) declares **M named outputs known at authoring time** (e.g. Prozorro winner → `{edrpou, amount, legalName}`; a person-parse → `{surname, given_name}`). Each output is its own `column` carrying a distinct `output_slot` (§2); `exec()` returns the position-aligned list `[{value, status, citation}, …]` of length M (§3). One row → **M cells across M columns**, each independently typed (§5), cited (§9), and cached — `cache_key` includes `output_slot` (§4 step 6) precisely so the M sibling columns don't collide on one key. M is part of the recipe identity: changing it is a `recipe_version` bump (§7), never a per-row surprise. All M columns share one `run` and one input wavefront (§4 step 5) — they're produced together, so they go blocked/ready together.

   **(b) Variable, runtime N → list-in-cell (Enumerate / multi-valued fact).** When N is **not known until the recipe runs on a given row** (Enumerate "list all X", founders, multi-lot awards, §16 #3), the N values live in **one cell** of column A as a list `value_jsonb` (§2, "may be list"; §16 #2), with `citation_jsonb` an **array aligned per item** (§7/§9) — one column, one cell, N items. A single `cell.status` covers the operation (§5); an **empty list is a terminal-empty status** (`NotFound` / `InsufficientData`), never a silent `[]` posing as `Answered` (Principle 4). Different rows in column A carry different-length lists — that's expected and is why this can't be modeled as case (a).

   **What explicitly does NOT happen** (so nobody re-invents it): a runtime-variable N never **auto-explodes into N child rows** (breaks the recipe contract §3 and per-row isolation — *rejected, not deferred*, §16 #2) and never **auto-fans-out into N ad-hoc columns** (column count is fixed by the schema in case (a), not discovered from a row's data). If a downstream step must act on *each* of the N items — verify each entity against YouControl, connect each — that is an **explicit, user-invoked op over the selected list items** (same pattern as §8 Cross-row connect / §16 #2, #7), not implicit engine behavior.

   **Downstream consumption of a list-in-cell column.** A recipe depending on a case-(b) column receives the **whole list** in `row_context` and owns its own semantics — fold/aggregate → scalar (§6 Aggregate/Fold), summarize the list, count it. The engine does **not** implicitly map a downstream recipe over the items: one input cell → one downstream cell, preserving per-row isolation (§3). Per-item processing is always the explicit op above. (Case (a) is unaffected — a downstream recipe just depends on whichever of the M sibling columns it needs, like any other column.)
