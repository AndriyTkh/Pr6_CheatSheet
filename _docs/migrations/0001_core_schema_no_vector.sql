-- 0001_core_schema.sql — NO VECTOR variant (pgvector not installed)
...
CREATE TABLE cell (
  ...
  cache_key      text,                                    -- NULL = non-hittable
  run_id         uuid REFERENCES run(id),
  version        bigint NOT NULL DEFAULT nextval('cell_version_seq'),
  updated_at     timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (row_id, column_id)
);
CREATE INDEX cell_col_status_idx ON cell(column_id, status);
CREATE INDEX cell_cache_key_idx  ON cell(cache_key);
CREATE INDEX cell_version_idx    ON cell(version);

-- =====================================================================
-- DOCUMENT + CHUNK (§7 doc modes, §9 citations, §11 external gate)
-- chunk table SKIPPED — requires pgvector extension (install later)
-- =====================================================================
CREATE TABLE document (
  id             uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  case_id        uuid NOT NULL REFERENCES "case"(id) ON DELETE CASCADE,
  row_id         uuid REFERENCES "row"(id) ON DELETE CASCADE,
  url            text,
  doc_type       text,
  format         text,
  ...
  external_ok    boolean NOT NULL DEFAULT false,         -- §11 HARD GATE
  created_at     timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE cross_row_result (
  ...
  input_versions_jsonb jsonb NOT NULL DEFAULT '{}',
